...
export const recipeFormSchema = z
  .object({
    ...
    estimatedPreparationMinutes: z.coerce
      .number()
      .int()
      .positive()
      .nullable()
    ...
    instructions: z.array(
      z.object({
        id: z.string().uuid().optional(),
        description: z.string().trim().min(1, 'Escribe el paso.').max(2000),
      }),
    ),
  })
  .superRefine((value, context) => {
    ...
    value.ingredients.forEach((ingredient, index) => {
      if (ingredient.unit === 'SERVING' && !ingredient.servingId)
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Selecciona una porción.',
          path: ['ingredients', index, 'servingId'],
        });
      if (ingredient.unit !== 'SERVING' && ingredient.servingId)
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'La porción solo aplica a esta unidad.',
          path: ['ingredients', index, 'servingId'],
        });
      ...
      if (keys.has(key))
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'No repitas el mismo alimento con la misma unidad.',
...
export const plannedMealFormSchema = z
  .object({
    date: z
      .string()
  ...
  .superRefine((value, context) => {
    if (value.source === 'RECIPE' && !value.recipeId)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['recipeId'],
        message: 'Selecciona una receta.',
      });
    if (value.source === 'PREVIOUS_MEAL' && !value.previousMealId)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['previousMealId'],
        message: 'Selecciona una comida anterior.',
      });
    if (
      ['FREE_MEAL', 'RESTAURANT', 'DELIVERY'].includes(value.source) &&
      !value.nameSnapshot?.trim()
    )
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['nameSnapshot'],
...
export type ParticipantQuantityValues = z.infer<
  typeof participantQuantitySchema
>;
