...
export function toRecipe(value: unknown): Recipe {
  ...
  return {
    ...
    id: String(source.id ?? ''),
    ingredients: Array.isArray(source.ingredients) ? source.ingredients.map((ingredient, index) => { const item = asRecord(ingredient); return { foodId: String(item.foodId ?? ''), id: String(item.id ?? `ingredient-${index}`), notes: nullableString(item.notes), position: Number(item.position ?? index + 1), quantity: Number(item.quantity ?? 0), servingId: nullableString(item.servingId), unit: String(item.unit ?? '') }; }) : [],
...
export function toRecipeNutrition(value: unknown): RecipeNutrition {
  const source = asRecord(value);
  return {
    ingredients: Array.isArray(source.ingredients) ? source.ingredients.map((ingredient) => { const item = asRecord(ingredient); return { baseQuantity: Number(item.baseQuantity ?? 0), baseUnit: String(item.baseUnit ?? ''), foodId: String(item.foodId ?? ''), ingredientId: String(item.ingredientId ?? ''), nutrients: asNumberRecord(item.nutrients) }; }) : [],
    ...
    warnings: Array.isArray(source.warnings) ? source.warnings.map((warning) => { const item = asRecord(warning); return { code: String(item.code ?? ''), foodId: String(item.foodId ?? ''), ingredientId: String(item.ingredientId ?? ''), message: String(item.message ?? '') }; }) : [],
  };
}

function nullableString(value: unknown) { return value == null ? null : String(value); }
function asRecord(value: unknown): Record<string, unknown> { return value && typeof value === 'object' ? value as Record<string, unknown> : {}; }
function asNumberRecord(value: unknown): Record<string, number> { const source = asRecord(value); return Object.fromEntries(Object.entries(source).map(([key, amount]) => [key, Number(amount)])); }
