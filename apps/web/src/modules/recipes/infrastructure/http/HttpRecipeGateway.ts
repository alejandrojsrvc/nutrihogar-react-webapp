...
interface RecipeApiClient {
  GET(
    ...
    options: {
      params: {
        path: { householdId?: string; recipeId?: string };
  ...
  PATCH(
    path: string,
    options: { params: { path: { recipeId: string } }; body: unknown },
  ): Promise<ApiResult<unknown>>;
  DELETE(
    path: string,
    options: { params: { path: { recipeId: string } } },
  ): Promise<ApiResult<unknown>>;
}
...
export class HttpRecipeGateway implements RecipeGateway {
  ...
  async create(householdId: string, input: CreateRecipeInput): Promise<Recipe> {
    return this.request(
      () =>
        (this.apiClient as unknown as RecipeApiClient).POST(
  ...
  async update(recipeId: string, input: UpdateRecipeInput): Promise<Recipe> {
    return this.request(
      () =>
        (this.apiClient as unknown as RecipeApiClient).PATCH(
          `/api/recipes/${recipeId}`,
          { params: { path: { recipeId } }, body: input },
        ),
      'actualizar',
    );
  }
  ...
  async getById(recipeId: string): Promise<Recipe> {
    return this.request(
      () =>
        (this.apiClient as unknown as RecipeApiClient).GET(
          `/api/recipes/${recipeId}`,
  ...
  async archive(recipeId: string): Promise<void> {
    try {
      const result = await (
        this.apiClient as unknown as RecipeApiClient
      ).DELETE(`/api/recipes/${recipeId}`, { params: { path: { recipeId } } });
  ...
  async list(
    ...
    try {
      const result = await (this.apiClient as unknown as RecipeApiClient).GET(
        `/api/households/${householdId}/recipes`,
        {
          params: {
            path: { householdId },
      ...
      if (!source)
        throw new ApiClientError('unknown', 'La API no devolvio las recetas.');
      return {
        items: Array.isArray(source.items) ? source.items.map(toRecipe) : [],
        limit: Number(source.limit ?? criteria.limit),
        page: Number(source.page ?? criteria.page),
    ...
    try {
      ...
      if (!result.data)
        throw new ApiClientError(
          'unknown',
          `La API no devolvio la receta al ${action}la.`,
        );
