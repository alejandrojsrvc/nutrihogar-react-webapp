...
describe('HttpPreparedBatchGateway', () => {
  it('loads operational details from the backend contract', async () => {
      ...
      async (input, init) => {
        ...
        return new Response(
          JSON.stringify({
            ...
            availability: {
              availableWeight: 500,
              finalCookedWeight: 1000,
              servedWeight: 400,
              storedLeftoverWeight: 100,
    ...
    const details = await new HttpPreparedBatchGateway(apiClient).getDetails(
      'batch-1',
    );
    expect(new URL(request?.url ?? '').pathname).toBe(
      '/api/prepared-batches/batch-1/details',
    );
