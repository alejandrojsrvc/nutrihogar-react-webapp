...
} from '../../application/ports/ServedPortionGateway';
type Result = { data?: unknown; error?: unknown; response?: Response };
interface Client {
  POST(
    path: string,
    options: { params: { path: { batchId: string } }; body: unknown },
  ): Promise<Result>;
}
export class HttpServedPortionGateway implements ServedPortionGateway {
  ...
  async serve(
    ...
    try {
      ...
      return result.data as ServePortionsResult;
    } catch (error) {
