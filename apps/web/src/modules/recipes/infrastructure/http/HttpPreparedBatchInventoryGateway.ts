...
type PreviewResponse =
  components['schemas']['PreparedBatchInventoryPreviewResponseDto'];
type ConfirmRequest =
  components['schemas']['ConfirmPreparedBatchInventoryConsumptionRequestDto'];

interface Client {
  GET(
    path: string,
    options: { params: { path: { batchId: string } } },
  ): Promise<Result<PreviewResponse>>;
  POST(
    path: string,
    options: { params: { path: { batchId: string } }; body: ConfirmRequest },
...
function toPreview(
  ...
  return {
    alreadyConfirmed: Boolean(
      source.alreadyConfirmed ??
      source.confirmed ??
      ingredients.some((item) => item.status === 'CONFIRMED'),
    ),
    batchId,
    ingredients,
  };
...
function toIngredient(value: unknown): PreparedBatchInventoryIngredient {
  ...
  const availability = availabilityValue(
    ...
    source.status,
    availableQuantity,
    number(source.quantity ?? source.requiredQuantity),
    options.length,
  );
  ...
  return {
    ...
    name: string(
      source.ingredientName ?? source.name ?? source.foodName ?? 'Ingrediente',
    ),
    options,
    selectedInventoryItemId: nullableString(
      source.inventoryItemId ??
        source.selectedInventoryItemId ??
        options[0]?.inventoryItemId,
    ),
    status,
    unit: string(source.unit ?? source.requiredUnit),
    usedQuantity: number(
      source.usedQuantity ?? source.quantity ?? source.requiredQuantity,
    ),
  };
}
...
function toOption(value: unknown) {
  ...
  return {
    availableQuantity: number(
      source.availableQuantity ?? source.currentQuantity ?? source.quantity,
    ),
...
function availabilityValue(
  ...
  required: number,
  optionCount: number,
): PreparedBatchInventoryIngredient['availability'] {
...
function statusValue(
  value: unknown,
  availability: PreparedBatchInventoryIngredient['availability'],
): PreparedBatchInventoryIngredient['status'] {
