  ...
  if (details.isError || !details.data)
    return (
      <p className="page-section" role="alert">
        No se pudo cargar el detalle de la preparación.
      </p>
    );
    ...
    <section className="page-section" aria-labelledby="prepared-batch-title">
      ...
      <dl className="recipe-detail-meta">
        <div>
          <dt>Peso cocido</dt>
          <dd>
            {batch.finalCookedWeight ?? 'Pendiente'}
            {batch.finalCookedWeight ? ' g' : ''}
          </dd>
        </div>
        <div>
          <dt>Servido</dt>
          <dd>{availability?.servedWeight ?? 'No disponible'} g</dd>
        </div>
        <div>
          <dt>Disponible</dt>
          <dd>{availability?.availableWeight ?? 'No disponible'} g</dd>
        </div>
      </dl>
      <section className="recipe-detail-section">
        <h2>Ingredientes reales</h2>
        <ul>
          {batch.ingredients.map((ingredient) => (
            <li key={ingredient.id}>
              {ingredient.foodNameSnapshot ?? ingredient.foodId}:{' '}
        ...
        {servedPortions.length ? (
          <ul>
            {servedPortions.map((portion) => (
              <li key={portion.id}>
                <span>
                  {portion.adultProfileId}: {portion.servedWeight} g ·{' '}
                  {portion.status}
                </span>
                {portion.consumedWeight == null ? (
                  ...
                  <small>{portion.consumedWeight} g consumidos</small>
                )}
              </li>
            ))}
          </ul>
        ) : (
          <p>Aún no hay porciones.</p>
        )}
      </section>
      <section className="recipe-detail-section">
        <h2>Sobrantes</h2>
        {leftovers.length ? (
          <ul>
            {leftovers.map((leftover) => (
              <li key={leftover.id}>
                <Link to={`/app/sobrantes/${leftover.id}`}>
                  {leftover.availableWeight} g · {leftover.status}
                </Link>
              </li>
            ))}
          </ul>
        ) : (
          <p>Aún no hay sobrantes.</p>
        )}
        {available ? (
          <Link
            className="button button--secondary"
            to={`/app/preparaciones/${batch.id}/sobrante`}
          >
            Guardar sobrante
          </Link>
        ) : null}
      ...
      {batch.status === 'FINALIZED' ? (
        <Link
          className="button button--secondary"
          to={`/app/preparaciones/${batch.id}/inventario`}
        >
          Revisar consumo de ingredientes
        </Link>
      ) : null}
      {batch.status === 'FINALIZED' ? (
        <Link
