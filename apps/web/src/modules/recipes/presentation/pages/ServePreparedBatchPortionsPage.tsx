...
export function ServePreparedBatchPortionsPage() {
  ...
  if (details.isPending || profiles.isPending)
    return (
      <p className="page-section" role="status">
        Cargando porciones...
      </p>
    );
  if (details.isError || !details.data || profiles.isError)
    return (
      <p className="page-section" role="alert">
        No se pudo cargar la preparación o los integrantes.
      </p>
    );
  const available = details.data.availability?.availableWeight ?? 0;
  const total = rows.reduce(
    (sum, row) => sum + (Number(row.servedWeight) || 0),
    0,
  );
  const invalid =
    !rows.some((row) => Number(row.servedWeight) > 0) || total > available;
  return (
    <section className="page-section">
      ...
      <PageHeader
        ...
        description={`Disponible: ${available} g`}
      />
      <div className="portion-distribution">
        {rows.map((row, index) => (
          <div className="recipe-ingredient-row" key={index}>
            <div className="form-field">
              <label htmlFor={`portion-profile-${index}`}>Adulto</label>
              <select
      ...
      <p
        className={total > available ? 'form-field__error' : 'supporting-text'}
      >
        Total asignado: {total} g · Restante: {available - total} g
      </p>
      {serve.isError ? (
        <p role="alert">
          No se pudo servir. La disponibilidad pudo cambiar; revisa las
          cantidades.
        </p>
      ) : null}
      <div className="recipe-form__actions">
        ...
        <button
          ...
          disabled={invalid || serve.isPending}
          onClick={() =>
            serve.mutate({
              batchId: batchId!,
              input: {
                portions: rows
                  .filter((row) => Number(row.servedWeight) > 0)
                  .map((row) => ({
          ...
          type="button"
        >
          {serve.isPending ? 'Guardando...' : 'Confirmar porciones'}
        </button>
      </div>
    </section>
  );
