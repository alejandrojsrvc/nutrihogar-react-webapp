export type PreparedBatchStatus =
  'DRAFT' | 'INGREDIENTS_CONFIRMED' | 'FINALIZED' | 'CANCELLED';
export interface PreparedBatchIngredient {
  id: string;
  foodId: string;
...
export interface ServedPortion {
  id: string;
  ...
  remainder: { weight: number; disposition: string; createdAt: string } | null;
  mealId: string | null;
  nutritionSnapshot: Record<string, number>;
}
export interface PreparedFoodLeftover {
  id: string;
...
export interface PreparedBatchDetails {
  batch: PreparedBatch;
