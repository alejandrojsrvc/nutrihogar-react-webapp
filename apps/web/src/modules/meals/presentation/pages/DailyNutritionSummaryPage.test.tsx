  ...
  it('changes adult and renders the empty day state', async () => {
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (input, init) => {
      ...
      if (request.url.includes('/adult-profiles'))
        return jsonResponse([
          profile('profile-1', 'Alejandro'),
          profile('profile-2', 'Sofia'),
        ]);
      return jsonResponse({ status: 'ok' });
    });

    renderRoute(
      '/app/resumen/2026-07-29',
    ...
    await waitFor(() =>
      expect(
        requests.some((request) =>
          request.url.includes('/profile-2/daily-nutrition-summary'),
        ),
      ).toBe(true),
    );
  });
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (input, init) => {
      ...
      if (request.url.includes('daily-nutrition-summary'))
        return jsonResponse({
          ...
          meals: [
            {
              consumedAt: '2026-07-29T13:00:00.000Z',
              id: 'meal-1',
              mealType: 'LUNCH',
      ...
      if (request.url.includes('/adult-profiles'))
        return jsonResponse([profile('profile-1', 'Alejandro')]);
      return jsonResponse({ status: 'ok' });
    });
