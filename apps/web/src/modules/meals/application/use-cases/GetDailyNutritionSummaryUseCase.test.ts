...
describe('GetDailyNutritionSummaryUseCase', () => {
  it('loads the summary through its gateway', async () => {
    ...
    await expect(new GetDailyNutritionSummaryUseCase(gateway).execute('profile-1', '2026-07-29')).resolves.toBe(summary);
    expect(gateway.getByProfileAndDate).toHaveBeenCalledWith('profile-1', '2026-07-29');
  });
});
