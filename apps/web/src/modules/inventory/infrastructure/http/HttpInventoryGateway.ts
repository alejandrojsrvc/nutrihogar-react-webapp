...
import {
  toInventoryItem,
  toInventoryMovement,
} from '../mappers/InventoryApiMapper';
...
interface Client {
  GET(
    path: string,
    options?: {
      params: { path: Record<string, string>; query?: InventoryFilters };
    },
  ): Promise<Result>;
  POST(
    path: string,
...
export class HttpInventoryGateway implements InventoryGateway {
  ...
  async list(householdId: string, filters: InventoryFilters = {}) {
    ...
    const items = Array.isArray(source.items)
      ? source.items.map(toInventoryItem)
      : [];
    return {
      items,
    ...
    return toInventoryItem(
      await this.request(() =>
        (this.apiClient as unknown as Client).POST(
          ...
          {
            body: { ...input, occurredAt: input.occurredAt?.toISOString() },
            params: { path: { inventoryItemId } },
          },
        ),
      ),
  ...
  async consumePrepared(
    ...
    return toRegisteredMeal(
      await this.request(() =>
        (this.apiClient as unknown as Client).POST(
          `/api/inventory/items/${inventoryItemId}/consume-prepared-food`,
          {
            body: { ...input, consumedAt: input.consumedAt.toISOString() },
            params: { path: { inventoryItemId } },
          },
        ),
      ),
    ...
    return toInventoryItem(
      await this.request(() =>
        (this.apiClient as unknown as Client).PATCH(
          `/api/inventory/items/${inventoryItemId}`,
          {
            body: { ...input, expiresAt: input.expiresAt?.toISOString() },
            params: { path: { inventoryItemId } },
          },
        ),
      ),
  ...
  async listMovements(inventoryItemId: string) {
    ...
    return Array.isArray(source.items)
      ? source.items.map(toInventoryMovement)
      : [];
  }
