...
} from '../../application/ports/InventorySyncGateway';
...
interface Client {
  POST(
    path: string,
    options: { params: { path: { householdId: string } }; body: SyncRequest },
  ...
  async synchronize(
    ...
    try {
      const result = await (this.apiClient as unknown as Client).POST(
        ...
        {
          body: { deviceId, operations: operations.map(toSyncOperation) },
      ...
      const processedSnapshots: Record<
        string,
        ReturnType<typeof toInventoryItem>
      > = {};
      const processed = Array.isArray(source.processed)
        ? source.processed.map((item) => {
            const operation = record(item);
            const operationId = String(operation.operationId ?? '');
            if (operation.snapshot)
              processedSnapshots[operationId] = toInventoryItem(
                operation.snapshot,
              );
            return operationId;
          })
      ...
      const conflicts = Array.isArray(source.conflicts)
        ? source.conflicts.map((item) => {
            const conflict = record(item);
            return {
              conflictCode:
                conflict.conflictCode == null
                  ? null
                  : (String(
                      conflict.conflictCode,
                    ) as InventorySyncConflict['conflictCode']),
              operationId: String(conflict.operationId ?? ''),
              reason: conflict.reason == null ? null : String(conflict.reason),
              resultingVersion:
                conflict.resultingVersion == null
                  ? null
                  : Number(conflict.resultingVersion),
              retryable: Boolean(conflict.retryable),
              snapshot: conflict.snapshot
                ? toInventoryItem(conflict.snapshot)
                : null,
            };
          })
        : [];
      return {
        ...
        processed,
        processedSnapshots,
        snapshot: source.snapshot ? toInventoryItem(source.snapshot) : null,
      };
    } catch (error) {
...
function toSyncOperation(
  operation: PendingInventoryOperation,
): components['schemas']['InventorySyncOperationRequestDto'] {
  return {
    allowLastWriteWins: operation.allowLastWriteWins,
