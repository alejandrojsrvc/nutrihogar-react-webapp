  ...
  async synchronize(householdId: string, deviceId: string, operations: PendingInventoryOperation[]) {
    try {
      const result = await (this.apiClient as unknown as Client).POST(
        ...
        { body: { deviceId, operations: operations.map(toSyncOperation) }, params: { path: { householdId } } },
      );
      if (result.error !== undefined) throw normalizeApiError(result.error, result.response);
      const source = record(result.data);
      const processedSnapshots: Record<string, ReturnType<typeof toInventoryItem>> = {};
      const processed = Array.isArray(source.processed) ? source.processed.map((item) => {
        const operation = record(item);
        const operationId = String(operation.operationId ?? '');
        if (operation.snapshot) processedSnapshots[operationId] = toInventoryItem(operation.snapshot);
        return operationId;
      }) : [];
      ...
      return { conflicts, processed, processedSnapshots, snapshot: source.snapshot ? toInventoryItem(source.snapshot) : null };
    } catch (error) {
...
function toSyncOperation(operation: PendingInventoryOperation): components['schemas']['InventorySyncOperationRequestDto'] {
  return {
