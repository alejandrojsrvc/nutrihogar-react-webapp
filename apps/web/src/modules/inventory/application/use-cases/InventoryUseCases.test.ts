...
describe('InventoryUseCases', () => {
  it('loads a remote snapshot and stores it locally when online', async () => {
    const gateway = {
      list: vi
        .fn()
        .mockResolvedValue({ items: [item], limit: 20, page: 1, total: 1 }),
    } as unknown as InventoryGateway;
    const local = localRepository();
    const connectivity: ConnectivityGateway = { isOnline: () => true };

    await expect(
      new LoadInventoryUseCase(gateway, local, connectivity).execute(
        'household-1',
      ),
  ...
  it('keeps remote inventory available when local caching fails', async () => {
    ...
    const local = localRepository();
    vi.mocked(local.saveSnapshot).mockRejectedValue(
  ...
  it('merges a filtered remote page into the complete local snapshot', async () => {
    const gateway = {
      list: vi.fn().mockResolvedValue({
        items: [{ ...item, id: 'item-2', name: 'Lentejas' }],
        limit: 1,
        page: 1,
        total: 2,
      }),
    ...
    await new LoadInventoryUseCase(gateway, local, connectivity).execute(
      'household-1',
      { query: 'lentejas' },
    );

    expect(local.saveSnapshot).toHaveBeenCalledWith('household-1', [
      item,
  ...
  it('loads an inventory detail from the local snapshot while offline', async () => {
    ...
    await expect(
      new GetInventoryItemUseCase(gateway, local, {
        isOnline: () => false,
      }).execute(item.id),
  ...
  it('queues an absolute adjustment while offline', async () => {
    ...
    await expect(
      new AdjustInventoryItemUseCase(gateway, local, connectivity).execute(
        'household-1',
        item,
        { quantity: 750, reason: 'Conteo', unit: 'GRAM' },
      ),
    ).resolves.toMatchObject({ currentQuantity: 750 });
    expect(local.saveOperation).toHaveBeenCalledWith(
      'household-1',
      expect.objectContaining({
        newQuantity: 750,
        payload: { reason: 'Conteo' },
  ...
  it('reactivates an optimistically adjusted depleted item and keeps the reason locally', async () => {
    ...
    const optimistic = await new AdjustInventoryItemUseCase(
      {} as InventoryGateway,
      local,
      { isOnline: () => false },
    ).execute('household-1', depleted, {
      quantity: 50,
      reason: 'Reposición',
      unit: 'GRAM',
    });
    ...
    expect(local.saveOperation).toHaveBeenCalledWith(
      'household-1',
      expect.objectContaining({ payload: { reason: 'Reposición' } }),
    );
  });

  it('synchronizes pending operations and marks processed ids', async () => {
    const local = localRepository();
    vi.mocked(local.listPendingOperations).mockResolvedValue([
      {
    ...
    const gateway: InventorySyncGateway = {
    ...
    await expect(
      new SynchronizeInventoryUseCase(
        gateway,
        local,
        connectivity,
        'device-1',
      ).execute('household-1'),
    ).resolves.toMatchObject({ processed: ['operation-1'] });
  ...
  it('keeps conflicts visible instead of deleting their operations', async () => {
    ...
    const gateway: InventorySyncGateway = {
      synchronize: vi.fn().mockResolvedValue({
        conflicts: [
          {
            conflictCode: 'INSUFFICIENT_BALANCE',
            operationId: 'operation-2',
    ...
    await new SynchronizeInventoryUseCase(
      gateway,
      local,
      connectivity,
      'device-1',
    ...
    expect(local.markOperationsConflicted).toHaveBeenCalledWith(
      ['operation-2'],
      {
        'operation-2': {
          conflictCode: 'INSUFFICIENT_BALANCE',
    ...
    expect(local.saveSyncResults).toHaveBeenCalledWith([
      expect.objectContaining({
        operationId: 'operation-2',
        status: 'CONFLICT',
      }),
  ...
  it('queues a waste exit offline and updates the local balance', async () => {
    ...
    await expect(
      new WasteInventoryItemUseCase(
        {} as InventoryGateway,
        local,
        connectivity,
      ).execute('household-1', item, { quantity: 125, unit: 'GRAM' }),
    ).resolves.toMatchObject({ currentQuantity: 375 });
    expect(local.saveOperation).toHaveBeenCalledWith(
      'household-1',
      expect.objectContaining({ movementType: 'WASTE', quantity: 125 }),
    );
  });
  ...
  it('queues an expiration exit offline and rejects quantities above the balance', async () => {
    const local = localRepository();
    const connectivity: ConnectivityGateway = { isOnline: () => false };
    const useCase = new ExpireInventoryItemUseCase(
      {} as InventoryGateway,
      local,
      connectivity,
    );
    ...
    await expect(
      useCase.execute('household-1', item, { quantity: 500, unit: 'GRAM' }),
    ).resolves.toMatchObject({ currentQuantity: 0 });
    expect(local.saveOperation).toHaveBeenCalledWith(
      'household-1',
      expect.objectContaining({ movementType: 'EXPIRATION' }),
    );
    await expect(
      useCase.execute('household-1', item, { quantity: 501, unit: 'GRAM' }),
    ).rejects.toThrow('no superar');
  });

  it('only allows prepared inventory items in the meal-producing flow', async () => {
    const gateway = {
      consumePrepared: vi.fn().mockResolvedValue({
    ...
    await expect(
      useCase.execute(prepared, {
        adultProfileId: 'profile-1',
        consumedAt: new Date(),
        mealType: 'LUNCH',
        quantity: 100,
      }),
    ).resolves.toMatchObject({ id: 'meal-1', mealType: 'LUNCH' });
