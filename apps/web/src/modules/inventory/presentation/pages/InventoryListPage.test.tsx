...
describe('InventoryListPage', () => {
  it('lists inventory and filters by type and search', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Tarta familiar' }),
    ).toBeInTheDocument();
  ...
  it('shows an empty state when the household has no inventory', async () => {
    ...
    expect(
      await screen.findByRole('heading', {
        name: 'Todavía no hay existencias',
      }),
    ).toBeInTheDocument();
  });
  ...
  it('initializes the special filter from the dashboard link', async () => {
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (input, init) => {
      ...
      if (pathname === '/api/households/household-1/inventory')
        return jsonResponse({ items: [], limit: 1, page: 1, total: 0 });
      throw new Error(`Unexpected request: ${request.url}`);
    });

    renderRoute(
      '/app/inventario?status=DEPLETED',
      createTestAuthGateway({ accessToken: 'test-token', userId: 'user-1' }),
    );
