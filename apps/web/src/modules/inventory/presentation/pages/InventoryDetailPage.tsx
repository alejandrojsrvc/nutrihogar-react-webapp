...
export function InventoryDetailPage() {
  ...
  const [action, setAction] = useState<'CONSUME' | 'WASTE' | 'EXPIRATION' | 'MINIMUM' | null>(null);
  ...
  const mutationError = consume.error ?? waste.error ?? expiration.error ?? update.error ?? archive.error;

  async function submitExit(kind: 'CONSUME' | 'WASTE' | 'EXPIRATION') {
    if (!quantity || Number(quantity) <= 0 || Number(quantity) > value.currentQuantity) return;
    const input = { quantity: Number(quantity), reason: reason.trim() || undefined, unit: value.unit, occurredAt: new Date(occurredAt) };
    try {
      if (kind === 'CONSUME') await consume.mutateAsync({ householdId, input, item: value });
      else if (kind === 'WASTE') await waste.mutateAsync({ householdId, input, item: value });
  ...
  return (
    <section className="page-section inventory-detail-page" aria-labelledby="inventory-detail-title">
      ...
      <div className="inventory-detail-actions"><span className={`status-badge${isArchived ? ' status-badge--danger' : ''}`}>{statusLabel(value.status)}</span>{!isArchived ? <><Link className="button button--primary" to={value.itemType === 'PREPARED_FOOD' ? `/app/inventario/${value.id}/consumir-preparado` : `/app/inventario/${value.id}/ajustar`}>{value.itemType === 'PREPARED_FOOD' ? 'Consumir preparado' : 'Ajustar cantidad'}</Link>{value.itemType !== 'PREPARED_FOOD' ? <><button className="button button--secondary" onClick={() => setAction(action === 'CONSUME' ? null : 'CONSUME')} type="button">Registrar consumo</button><button className="button button--secondary" onClick={() => setAction(action === 'WASTE' ? null : 'WASTE')} type="button">Registrar desperdicio</button><button className="button button--secondary" onClick={() => setAction(action === 'EXPIRATION' ? null : 'EXPIRATION')} type="button">Registrar vencimiento</button></> : null}</> : null}</div>
      ...
      {action === 'CONSUME' || action === 'WASTE' || action === 'EXPIRATION' ? <section className="inventory-detail-section inventory-action-form"><h2>{action === 'CONSUME' ? 'Registrar consumo' : action === 'WASTE' ? 'Registrar desperdicio' : 'Registrar vencimiento'}</h2>{action === 'EXPIRATION' ? <p role="note">Al confirmar, esta cantidad se retirará por vencimiento y no generará una comida.</p> : null}<div className="inventory-inline-form"><label htmlFor="inventory-exit-quantity">Cantidad disponible: {formatQuantity(value.currentQuantity, value.unit)}</label><input aria-describedby="inventory-exit-help" id="inventory-exit-quantity" inputMode="decimal" max={value.currentQuantity} min="0.1" onChange={(event) => setQuantity(event.target.value)} step="any" type="number" value={quantity} /><p className="supporting-text" id="inventory-exit-help">Saldo resultante: {formatQuantity(Math.max(value.currentQuantity - (Number(quantity) || 0), 0), value.unit)}</p><label htmlFor="inventory-exit-reason">Razón opcional</label><input id="inventory-exit-reason" onChange={(event) => setReason(event.target.value)} type="text" value={reason} /><label htmlFor="inventory-exit-date">Fecha y hora</label><input id="inventory-exit-date" onChange={(event) => setOccurredAt(event.target.value)} type="datetime-local" value={occurredAt} /><button className="button button--primary" disabled={consume.isPending || waste.isPending || expiration.isPending || !quantity || Number(quantity) > value.currentQuantity} onClick={() => void submitExit(action)} type="button">{consume.isPending || waste.isPending || expiration.isPending ? 'Guardando...' : 'Confirmar'}</button></div></section> : null}
