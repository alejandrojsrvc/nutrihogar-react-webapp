...
import type { PurchaseGateway, PurchaseInput, PurchaseFilters } from '../../application/ports/PurchaseGateway';
...
export class HttpPurchaseGateway implements PurchaseGateway {
  ...
  async list(householdId: string, filters: PurchaseFilters = {}): Promise<PurchaseListResult> {
    const result = await this.request(() => (this.apiClient as unknown as Client).GET(
      `/api/households/${householdId}/purchases`,
      { params: { path: { householdId }, query: filters as Record<string, unknown> } },
    ));
  ...
  async getById(purchaseId: string) {
    return toPurchase(await this.request(() => (this.apiClient as unknown as Client).GET(
  ...
  async update(purchaseId: string, input: Partial<PurchaseInput>) {
    return toPurchase(await this.request(() => (this.apiClient as unknown as Client).PATCH(
      `/api/purchases/${purchaseId}`,
