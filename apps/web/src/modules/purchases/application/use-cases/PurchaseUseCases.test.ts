...
describe('PurchaseUseCases', () => {
  it('does not create an empty purchase draft', () => {
    ...
    expect(() =>
      new CreatePurchaseUseCase(gateway).execute('household-1', {
        items: [],
  ...
  it('confirms a purchase through the gateway', async () => {
    const gateway = {
      confirm: vi.fn().mockResolvedValue({ id: 'purchase-1' }),
    } as unknown as PurchaseGateway;

    await expect(
      new ConfirmPurchaseUseCase(gateway).execute('purchase-1'),
