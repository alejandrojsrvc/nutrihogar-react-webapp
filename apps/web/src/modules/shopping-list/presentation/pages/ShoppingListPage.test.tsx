...
describe('ShoppingListPage', () => {
  it('shows pending items and distinguishes the purchased action from inventory changes', async () => {
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (input, init) => {
      ...
      if (pathname === '/api/households/household-1/shopping-list') return jsonResponse({ items: [{ id: 'shopping-1', name: 'Leche', quantity: 2, source: 'MANUAL', unit: 'L', purchased: false }] });
      if (pathname === '/api/shopping-list/items/shopping-1/mark-purchased') return jsonResponse({});
      throw new Error(`Unexpected request: ${request.url}`);
    });

    renderRoute('/app/lista-de-compras', createTestAuthGateway({ accessToken: 'test-token', userId: 'user-1' }));
