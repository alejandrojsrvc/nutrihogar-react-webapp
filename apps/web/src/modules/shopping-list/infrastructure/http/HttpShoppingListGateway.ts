...
} from '../../application/ports/ShoppingListGateway';
import {
  toShoppingList,
  toShoppingListItem,
} from '../mappers/ShoppingListApiMapper';
    ...
    return toPurchase(
      await this.request(() =>
        (this.apiClient as unknown as Client).POST(
          ...
          {
            params: { path: { householdId } },
            body: { ...input, purchaseDate: input.purchaseDate.toISOString() },
          },
        ),
      ),
    );
