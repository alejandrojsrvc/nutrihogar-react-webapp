...
describe('ShoppingListUseCases', () => {
  it('rejects non-positive manual items', async () => {
    ...
    expect(() =>
      new AddShoppingListItemUseCase(gateway).execute('household-1', {
        name: 'Arroz',
        quantity: 0,
        unit: 'kg',
      }),
  ...
  it('requires selected items before converting to a purchase', async () => {
    const gateway = {
      convertToPurchase: vi.fn(),
    } as unknown as ShoppingListGateway;

    expect(() =>
      new ConvertShoppingListToPurchaseUseCase(gateway).execute('household-1', {
        itemIds: [],
        items: [],
