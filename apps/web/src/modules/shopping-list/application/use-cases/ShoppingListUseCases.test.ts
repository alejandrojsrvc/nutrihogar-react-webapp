...
describe('ShoppingListUseCases', () => {
  it('rejects non-positive manual items', async () => {
    ...
    await expect(new AddShoppingListItemUseCase(gateway).execute('household-1', { name: 'Arroz', quantity: 0, unit: 'kg' })).rejects.toThrow('mayor que cero');
  ...
  it('requires selected items before converting to a purchase', async () => {
    ...
    await expect(new ConvertShoppingListToPurchaseUseCase(gateway).execute('household-1', {
      itemIds: [],
      items: [],
      purchaseDate: new Date('2026-08-01T12:00:00.000Z'),
      storeName: 'Mercado',
      total: 100,
    })).rejects.toThrow('al menos un elemento');
