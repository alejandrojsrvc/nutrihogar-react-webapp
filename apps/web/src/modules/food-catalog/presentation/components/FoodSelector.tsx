...
import { FullscreenDialog } from '../../../../shared/presentation/components/Overlay';
...
export function FoodSelector({
  ...
  const [measurementMethod, setMeasurementMethod] =
    useState<MeasurementMethod>('WEIGHED');
  ...
  const selectedFood =
    detail.data ?? foods.data?.items.find((food) => food.id === selectedFoodId);
  const selectedServing =
    detail.data?.servings.find((serving) => serving.id === selectedServingId) ??
    detail.data?.servings[0];
  ...
  function confirmSelection() {
    ...
    onSelect({
      ...
      servingEquivalent:
        selectedServing?.equivalentGrams ??
        selectedServing?.equivalentMilliliters,
      servingId: unit === 'SERVING' ? selectedServing?.id : undefined,
      unit,
    });
  }
  ...
  return (
    <FullscreenDialog
      description="Busca un alimento, revisa la cantidad y agrégalo cuando esté listo."
      footer={
        <button
          className="button button--primary"
          disabled={!selectedFood || Number(quantity) <= 0}
      ...
      <div className="food-selector">
        <div className="food-selector__filters">
          <div className="form-field">
            <label htmlFor="selector-query">Nombre</label>
            <input
              autoFocus
              id="selector-query"
          ...
          <div className="form-field">
            <label htmlFor="selector-preparation">Preparación</label>
            <select
              id="selector-preparation"
              onChange={(event) =>
                setPreparationState(
                  event.target.value as typeof preparationState,
                )
              }
            ...
            >
              ...
              <option value="READY_TO_EAT">Listo para comer</option>
            </select>
          </div>
        </div>
        ...
        {!foods.isPending && !foods.isError && foods.data?.items.length === 0 ? (
          <p role="status">No encontramos alimentos.</p>
        ) : null}
        <div className="food-selector__results">
          {foods.data?.items.map((food) => (
            <button
              className={`food-selector__result${food.id === selectedFoodId ? ' is-selected' : ''}`}
              key={food.id}
            ...
            >
              ...
              <small>
                {formatAmount(food.energyKcal)} kcal ·{' '}
                {formatAmount(food.proteinGrams)} g de proteína
              </small>
            </button>
          ))}
        </div>
        {selectedFood ? (
          <div className="food-selector__selection">
            ...
            <div className="form-field">
              <label htmlFor="selected-quantity">Cantidad</label>
              <input
            ...
            <div className="form-field">
              <label htmlFor="selected-unit">Unidad</label>
              <select
                id="selected-unit"
                onChange={(event) => {
              ...
              >
                ...
                {detail.data?.servings.length ? (
                  <option value="SERVING">Porción</option>
                ) : null}
              </select>
            </div>
            {unit === 'SERVING' && detail.data?.servings.length ? (
              <div className="form-field">
                <label htmlFor="selected-serving">Porción disponible</label>
                <select
                  id="selected-serving"
                  onChange={(event) => setSelectedServingId(event.target.value)}
                  value={selectedServing?.id ?? ''}
                >
                  {detail.data.servings.map((serving) => (
            ...
            <div className="form-field">
              <label htmlFor="selected-method">Método de medición</label>
              <select
                id="selected-method"
                onChange={(event) =>
              ...
              >
                ...
                <option value="SERVING">Porción</option>
                <option value="UNIT">Unidad</option>
                <option value="APPROXIMATED">Aproximado</option>
              </select>
            </div>
          </div>
        ) : null}
