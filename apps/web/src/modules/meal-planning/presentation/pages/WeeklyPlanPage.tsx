...
export function WeeklyPlanPage() {
  ...
  return <section className="page-section meal-planning" aria-labelledby="weekly-plan-title">
    ...
    {!value ? <div className="empty-state"><h2>Aún no hay plan para esta semana</h2><p>Créalo cuando quieras organizar las comidas del hogar.</p><button className="button button--primary" disabled={create.isPending} onClick={createPlan} type="button"><Plus size={18} />{create.isPending ? 'Creando...' : 'Crear plan semanal'}</button>{create.isError ? <p role="alert">No se pudo crear el plan semanal. Inténtalo nuevamente.</p> : null}</div> : <>
       <p aria-live="polite" className="meal-planning__announcement">Plan semanal cargado. {value.meals.length ? `${value.meals.length} comida${value.meals.length === 1 ? '' : 's'} planificada${value.meals.length === 1 ? '' : 's'}.` : 'Todavía no hay comidas.'}</p><nav className="meal-planning__plan-links" aria-label="Resumen del plan"><Link to={`/app/plan-semanal/${value.id}/requerimientos`}>Ingredientes requeridos</Link><Link to={`/app/plan-semanal/${value.id}/comparacion-inventario`}>Comparar inventario</Link><Link to={`/app/plan-semanal/${value.id}/adherencia`}>Ver adherencia</Link></nav>
...
function PlannedMealCard({ meal, planId }: { meal: PlannedMeal; planId: string }) {
  return <article className="meal-planning__card"><strong>{meal.name ?? sourceLabel(meal.source)}</strong><span>{statusLabels[meal.status]} · {sourceLabel(meal.source)}</span><span>{meal.participants.length} participante{meal.participants.length === 1 ? '' : 's'}</span><Link to={`/app/plan-semanal/${planId}/comidas/${meal.id}/editar`}>Editar</Link><div className="meal-planning__quick-actions"><Link to={`/app/plan-semanal/${planId}/comidas/${meal.id}/participantes`}>Participantes</Link><Link to={`/app/plan-semanal/${planId}/comidas/${meal.id}/cantidades`}>Cantidades</Link>{meal.source === 'RECIPE' && meal.status === 'PLANNED' ? <Link to={`/app/plan-semanal/${planId}/comidas/${meal.id}/preparar`}>Preparar</Link> : null}{meal.mealId ? <Link to={`/app/comidas/${meal.mealId}`}>Ver consumo</Link> : meal.status === 'PLANNED' && meal.source !== 'EMPTY' ? <Link to={`/app/comidas/nueva?mealType=${meal.type}&date=${meal.date}&plannedMealId=${meal.id}`}>Registrar consumo</Link> : null}</div></article>;
}
