...
describe('MealPlanningApiMapper', () => {
      ...
      meals: [
        {
          ...
          nameSnapshot: 'Avena',
          status: 'PLANNED',
          participants: [{ id: 'participant', adultProfileId: 'adult' }],
          position: 2,
        },
      ],
    });
  ...
  it('maps decimal nutrition responses to numbers without changing units', () => {
    expect(
      toQuantitySuggestion({
        ...
        goalValidFrom: '2026-08-01T00:00:00Z',
        participantId: 'participant',
        quantity: '1.25',
        targetCalories: '650.000',
        unit: 'SERVING',
      }),
    ).toMatchObject({ quantity: 1.25, targetCalories: 650, unit: 'SERVING' });
    expect(
      toWeeklyRequirements({
        items: [
          { foodId: 'food', name: 'Arroz', required: '1200.000', unit: 'GRAM' },
        ],
        warnings: ['Sin inventario'],
      }).items[0].required,
    ).toBe(1200);
    expect(
      toInventoryComparison({
  ...
  it('maps backend adherence values without calculating nutrition locally', () => {
    const summary = toAdherenceSummary({
      counts: {
        cancelled: 0,
        consumed: 2,
        planned: 3,
      ...
      nutrition: {
        ...
        plannedProtein: '90',
        proteinPercentage: '77.77',
      },
      percentages: { consumed: '66.67', unplanned: '0' },
      warnings: [],
      weekStart: '2026-08-03',
      weeklyPlanId: 'plan-1',
    });
    expect(summary).toMatchObject({
      nutrition: { caloriePercentage: 80, plannedCalories: 2000 },
      percentages: { consumed: 66.67 },
      weeklyPlanId: 'plan-1',
    });
  });
});
